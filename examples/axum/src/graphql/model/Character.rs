...
impl Character {
    ...
    async fn friends(&self, ctx: &FieldContext<'_>, first: Option<i64>, after: Option<ID>) -> Result<FriendsConnection, Error> {
        match self {
            Character::Human(obj) => {
                Ok(obj.friends(&ctx, first, after).await?)
            }
            Character::Droid(obj) => {
                Ok(obj.friends(&ctx, first, after).await?)
            }
